...
internal class AsViewPagerIndicator(context: Context, attr: AttributeSet) : View(context, attr) {
    ...
    init {
        ...
        try {
            textSize = tr.getDimensionPixelSize(R.styleable.AsViewPagerIndicator_asTextSize, sp2px(14f))
    ...
    private fun measureWidth(measureSpec: Int): Int {
        ...
        if (specMode == View.MeasureSpec.EXACTLY)
            ...
            if (listItems.size == 0) {
                ...
                result = paddingLeft + paddingRight + itemSpace * (listItems.size - 1) + listItems.sumBy { contentPaint.measureText(it).toInt() }
            }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.drawRoundRect(indicatorRectF, indicatorRectF.height(), indicatorRectF.height(), contentPaint)
    }

    private fun createRectF(): RectF {
        return RectF(0f, (paddingTop + verticalSpace).toFloat(), 0f, paddingTop + verticalSpace + itemHeight)
    }
    ...
    private fun reduceItemByAnimator(position: Int, isHasNext: Boolean) {
        ...
        if (isHasNext) {
            ...
            vp?.currentItem = vp?.childCount ?: 1 - 1
        }
        val left = rectF.left
    ...
    private fun scrollIndicatorByAnimator(rectF: RectF, newRectF: RectF) {
        ...
        animator.repeatCount = 0
        animator.duration = 600
        animator.addUpdateListener { animation -> scrollIndicator(rectF, newRectF, animation.animatedValue as Float) }
        animator.addListener(object : Animator.AnimatorListener {
            override fun onAnimationStart(animation: Animator) {
                isAnimatorRunning = true
            }
